"""Tests for the client's usage-reporting wiring: the settings block, the one-time notice, the
...
_SRC = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "client", "src")
...
_spec = importlib.util.spec_from_file_location("client_usage_reporting", os.path.join(_SRC, "usage_reporting.py"))
usage_reporting = importlib.util.module_from_spec(_spec)
sys.modules["client_usage_reporting"] = usage_reporting
...
class TestUsageReportingSettings(unittest.TestCase):
    ...
    def test_settings_file_is_next_to_the_client_directory(self):
        self.assertEqual(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "client", "settings.json"), SETTINGS_FILE)
    ...
    def test_notice_says_it_is_on_and_names_every_opt_out(self):
        self.assertTrue(FIRST_RUN_NOTICE.startswith("Usage reporting is on: the curabitur client sends"))
...
class TestStartupEvent(unittest.TestCase):
    ...
    def test_startup_event_reaches_the_configured_endpoint_with_name_and_version(self):
        ...
        with patch("client_usage_reporting.atexit"):
        ...
        self.assertEqual({"application": "curabitur", "name": "startup", "tags": {"version": VERSION, "role": "client"}},
                         request["body"])
    ...
    def test_environment_opt_out_wins_over_enabled_settings(self):
        ...
        for variable, value in (("DO_NOT_TRACK", "1"), ("TRACE_USAGE_REPORTING", "off")):
            with self.subTest(variable=variable):
                with patch("client_usage_reporting.atexit"), patch.dict(os.environ, {variable: value}):
                    client = startUsageReporting(self.settingsFile, lambda message: None)
    ...
    def test_start_never_raises_even_if_settings_loading_fails(self):
        with patch("client_usage_reporting.loadSettings", side_effect=RuntimeError("boom")):
            client = startUsageReporting(self.settingsFile, lambda message: None)
