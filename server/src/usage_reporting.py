"""Reports that the curabitur chat server was used to the trace service, and nothing else.

What is sent: the program's name (``curabitur``) and version with a ``startup``
event tagged ``role`` = ``server``, and a ``connected`` event with the same tag
...
block to ``server/settings.json`` and prints a one-line notice saying so and how to
...
# The program key the curabitur chat server ships with. Keys identify a program rather than
